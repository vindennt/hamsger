 ...
 * with local keystores having private keys and database receiving public
...
export async function verifyUserKeysExist(
  ...
  if (!bundleData) {
    ...
    if (insertError) {
      console.error("Failed to upload prekey bundle:", insertError);
    }

    // Generate 5 One-Time Prekeys and upload to database
    // TODO: change limit if higher usage
    ...
    // CASE 2: Bundle exists, verify we have local private keys
    const ik_pub = await keystore.get(`ik_pub_${userId}`);
    if (!ik_pub) {
      console.log("[Crypto Onboarding] Key bundle exists but no local keys");
      const ik = new KeyPair("IK_" + username);
      const spk = new KeyPair("SPK_" + username);
      const sigKP = new SigningKeyPair();
      const signature = sigKP.sign(spk.publicKey);
